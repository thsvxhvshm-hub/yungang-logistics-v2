...
app.put('/api/shipments/:id',auth,role('老板','操作员'),async(req,res)=>{try{const id=Number(req.params.id); const x=cleanShipment(req.body); const sets=fields.map((k,i)=>`${k}=$${i+1}`).join(','); const r=await pool.query(`UPDATE shipments SET ${sets},updated_by=$${fields.length+1},updated_at=NOW() WHERE id=$${fields.length+2} RETURNING *`,[...fields.map(k=>x[k]),req.user.id,id]); if(!r.rowCount)return res.status(404).json({error:'记录不存在'}); await log(req.user,'update','shipment',id,x);res.json(r.rows[0]);}catch(e){console.error(e);res.status(500).json({error:'修改失败'})}});
app.delete('/api/shipments/:id',auth,role('老板'),async(req,res)=>{try{const id=Number(req.params.id);const r=await pool.query('DELETE FROM shipments WHERE id=$1 RETURNING *',[id]);if(!r.rowCount)return res.status(404).json({error:'记录不存在'});await log(req.user,'delete','shipment',id,r.rows[0]);res.json({ok:true});}catch(e){console.error(e);res.status(500).json({error:'删除失败'})}});
